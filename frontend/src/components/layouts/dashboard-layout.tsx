...
const DashboardLayout = ({ children }: IAuthLayoutProps) => {
    ...
    const policy = useRef<RolePolicy>({
        CUSTOMER: ['accounts', 'managers'],
        ADMIN: ['managers', 'branches'],
        TELLER: ['accounts'],
        MANAGER: ['managers', 'accounts']
    })
    const loggedInUserRole: 'MANAGER' | 'TELLER' | 'CUSTOMER' | 'ADMIN' = JSON.parse(Cookies.get('user') as string).role;
            ...
            <main className="flex">
                <div className="bg-gray-900 sm:w-60 min-h-screen w-14 pt-4 transition-all">
                    ...
                    <ul className="mt-11 sidebar">
                        { policy.current[loggedInUserRole].map((item, i) => {
                            return (
                                <Link to={`/${item}`} key={i}>
                                    <li className="hover:bg-gray-800 cursor-pointer sm:justify-start px-4 h-12 flex items-center justify-center ">
                                        ...
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d={icons.current[item]} fill="#fff"/>
                                        </svg>
